      ...
      <nav className="bg-black rounded-2xl px-6 py-4 shadow-lg">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {navItems.map(item => <a key={item.name} href={item.href} className={`text-white hover:text-orange transition-colors duration-300 relative ${item.active ? 'font-semibold' : 'font-medium'}`}>
                {item.name}
                {item.active && <div className="absolute -bottom-1 left-0 w-full h-0.5 bg-orange rounded-full"></div>}
              </a>)}
          </div>
